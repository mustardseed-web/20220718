          ...
          <th class="column_updated_at">
            <form action="{{ route('update', ['todoId' => $todo->id]) }}" method="post">
              ...
              <select class="form-control" id="category-id" name="category_id">
                ...
                <option value="{{ $category->id }}" @if('category->category_id'==$todo->category->id) selected @endif>{{
                  $category->category_name }}</option>
                {{-- <option value="{{ $category->id }}" @if(old('category_name')==$todo->category->category_name)
                  selected
                  @endif>{{
                  $category->category_name }}</option> --}}
                @endforeach
